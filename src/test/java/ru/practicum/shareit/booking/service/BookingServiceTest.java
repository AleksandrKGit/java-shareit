...
class BookingServiceTest {
    ...
    static Stream<Arguments> incorrectDates() {
        ...
        return Stream.of(
                Arguments.of("start equals end", now, now),
                Arguments.of("start after end", now.plusSeconds(1), now)
        );
    }

    @ParameterizedTest(name = "{0}")
    @MethodSource("incorrectDates")
    void create_withStartNotBeforeEnd_shouldThrowValidationException(String testName, LocalDateTime start,
                                                                     LocalDateTime end) {
    ...
    @Test
    void readByOwner_withInvalidState_shouldThrowValidationException() {
        assertThrows(ValidationException.class, () -> service.readByOwner(userId, "", from, size));
    }
    ...
    void readByOwner_withEmptyBookingsList_shouldThrowNotFoundException(BookingState state) {
        ...
        assertThrows(NotFoundException.class, () -> service.readByOwner(userId, state.toString(), from, size));
    ...
    void readByOwner_shouldReturnDtoListOfSelectedOwnerBookings(BookingState state) {
        ...
        List<BookingDtoToClient> resultBookingListDto = service.readByOwner(userId, state.toString(), from, size);
    ...
    @Test
    void readByBooker_withInvalidState_shouldThrowValidationException() {
        assertThrows(ValidationException.class, () -> service.readByBooker(userId, "", from, size));
    }
    ...
    void readByBooker_withEmptyBookingsList_shouldThrowNotFoundException(BookingState state) {
        ...
        assertThrows(NotFoundException.class, () -> service.readByBooker(userId, state.toString(), from, size));
    ...
    void readByBooker_shouldReturnDtoListOfSelectedBookerBookings(BookingState state) {
        ...
        List<BookingDtoToClient> resultBookingListDto = service.readByBooker(userId, state.toString(), from, size);
